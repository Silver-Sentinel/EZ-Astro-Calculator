...
/// Fallback to GeoNames API if Photon fails (requires account setup)
/// NOTE: Replace "YOUR_USERNAME" with actual GeoNames username
async fn search_location_geonames(query: &str) -> Result<Vec<LocationResult>, String> {
    // This is a fallback - user must configure their own username
    let username = "YOUR_GEONAMES_USERNAME"; // User needs to replace this!
    
    if username == "YOUR_GEONAMES_USERNAME" {
        return Err("GeoNames username not configured. Please register at geonames.org and enable web services.".to_string());
    }
    ...
    if let Some(status) = data.get("status") {
        if let Some(msg) = status.get("message").and_then(|m| m.as_str()) {
            return Err(format!("GeoNames error: {}", msg));
        }
    }
